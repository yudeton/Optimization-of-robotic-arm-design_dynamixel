...
class RobotOptEnv(gym.Env):
    ...
    def __init__(self):
        super(RobotOptEnv, self).__init__()
        ...
        self.max_steps = 50  # 每个 episode 的最大步数
    ...
    def step(self, action):
        ...
        # 使用连续参数调整连杆长度
        length_change_L2 = continuous_params[0]  # 连杆2的长度增减
        length_change_L3 = continuous_params[1]  # 连杆3的长度增减

        self.std_L2 += length_change_L2
        self.std_L3 += length_change_L3

        # 限制连杆长度在允许范围内
        self.std_L2 = np.clip(self.std_L2, self.MIN_LENGTH, self.MAX_LENGTH)
        self.std_L3 = np.clip(self.std_L3, self.MIN_LENGTH, self.MAX_LENGTH)
    ...
    def time_step_spec(self):
        ...
        # 定义步骤类型的规格
        step_type_spec = array_spec.ArraySpec(
            shape=(),
            dtype=np.int32,
            name='step_type'
        )
