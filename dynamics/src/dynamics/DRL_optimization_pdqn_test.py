...
class PDQNAgent:
    def __init__(self, state_dim, action_dim, param_dim, buffer_size=50000, batch_size=64, gamma=0.99, lr_q=0.001, lr_p=0.001, log_dir=None):
        ...
        #time_step_spec = ts.time_step_spec(tf.TensorSpec(shape=(self.state_dim,), dtype=tf.float32))
        action_spec = env.action_spec()

        self._collect_data_spec = self._create_collect_data_spec(time_step_spec, action_spec)
        self.collect_policy = PDQNCollectPolicy(time_step_spec, action_spec, self)
    
    @property
    def collect_data_spec(self):
        return self._collect_data_spec
    
    def _create_collect_data_spec(self, time_step_spec, action_spec):
        return time_step_spec._replace(action=action_spec)
...
class PDQNCollectPolicy(TFPolicy):
    def __init__(self, time_step_spec, action_spec, agent):
        self._agent = agent
        super(PDQNCollectPolicy, self).__init__(
    ...
    def _action(self, time_step, policy_state, seed):
        action, policy_state = self._agent.collect_action(time_step, policy_state, seed)
        return policy_step.PolicyStep(action, policy_state)


    def _distribution(self, time_step, policy_state):
        raise NotImplementedError('This policy does not support distributions.')

'''
class PDQNCollectPolicy:
    def __init__(self, agent):
        self.agent = agent

    def action(self, time_step):
        state = time_step.observation.numpy()
        epsilon = 0.1  # 探索的概率，可以根据需要调整
        action, params = self.agent.choose_action(state[0], epsilon)
        return action, params
'''
#訓練過程
class PDQNTrainer:
